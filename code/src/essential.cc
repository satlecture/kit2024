/**
 * @brief Determines the variables essential for the satisfiability of a given formula.
 * @author Tomáš Balyo, Markus Iser
 */
...
int main(int argc, char **argv) {
	...
	if (result == 20) {
		std::cout << "The input formula is unsatisfiable." << std::endl;
		return 0;
	}

	std::vector<int> essentialVars;
	for (int var = 1; var <= formula.nVars(); var++) {
		std::cout << "Testing if variable " << var << " / " << formula.nVars() << " is essential for satisfiability: ";
	...
	std::cout << "Essential variables: ";
	for (int var : essentialVars) {
		std::cout << var << " ";
	}
	std::cout << std::endl;
