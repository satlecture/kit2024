...
\begin{alert}{Let's try it!}
	\begin{itemize}\setlength{\itemsep}{1em}
		\item Download and Build a SAT solver:
			\begin{itemize}
				\item \href{https://github.com/arminbiere/kissat}{Kissat}
				\item \href{https://github.com/arminbiere/cadical}{CaDiCaL}
				\item \href{https://github.com/niklasso/minisat}{Minisat}
				\item \dots
			\end{itemize}
...
\begin{itemize}
\item We often need to solve a sequence of similar SAT instances
\begin{itemize}
	\item for example planning as SAT, Sokoban, bounded model checking
	\item the instances share most of the clauses with their neighbors
\end{itemize}
\item Can we solve these sequences of instances more efficiently?
\pause
\item What is incremental SAT solving?
\begin{itemize}
	\item Clauses can be added to and removed from the SAT solver
\end{itemize}

\item Why not call the solver with the new formula every time?
\pause
\begin{itemize}
	\item The solver can remember learned clauses and other stuff (variable scores required for heuristics)
	\item (de)initialization overheads removed
\end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{Incremental Library Track -- IPASIR}
\begin{figure}[htp]
\centering
\includegraphics[scale=0.15]{figures/l01/ipa.jpeg}
\end{figure}
\begin{itemize}
\item Previously each SAT solver had a different incremental interface
\item For the 2015 SAT Race a unified interface was defined -- IPASIR
\item IPASIR = Re-entrant Incremental Satisfiability Application Program Interface (acronym reversed)
\item IPASIR has become a standard interface of incremental SAT solving, version 2 is in the works.
\end{itemize}
\end{frame}

\begin{frame}{IPASIR Overview}
\begin{itemize}
\item Based on Lingeling incremental interface
\item Clauses are added one literal at a time
\begin{itemize}
	\item To add $(x_1 \vee \overline{x_4})$ call \texttt{add(1); add(-4); add(0);}
\end{itemize}
\item<2-> You can call a SAT solver with a set of assumptions
\begin{itemize}
	\item Assumptions are basically temporary unit clauses
	\item Assumptions are cleared after each "solve" call
\end{itemize}
\item<3-> Clause removal is done via activation literals and assumptions
\begin{itemize}
...
\begin{itemize}
	\item signature -- return the name and version of the solver
	\item init -- initialize the solver, the pointer it returns is used for the rest of the functions
	\item add -- add clauses, one literal at a time
	\item assume -- add an assumption, the assumptions are cleared after a "solve" call
	\item solve -- solve the formula, return SAT, UNSAT or INTERRUPTED
	\item val -- return the truth value of a variable (if solve returned SAT)
	\item failed -- returns true if the given assumption was required for the unsatisfiability of
	the formula (if solver returned UNSAT)
\end{itemize}
\end{frame}

\begin{frame}{IPASIR Solver States}
\begin{center}
\includegraphics[scale=0.45]{figures/l01/ipasir.pdf}
\end{center}
\end{frame}

\begin{frame}{Example -- Essential Variables}
\begin{itemize}
\item For a satisfiable formula $F$, a variable $x$ is \emph{essential} if and only $x$ it has to be assigned (True
or False) in each satisfying assignment of $F$.
\item Task: find all the essential variables of a given formula
\item How to do it:
\begin{itemize}
