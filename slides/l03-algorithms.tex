...
\begin{block}{Minimize the Number of Unsatisfied Clauses}
	Start with a \highl{random complete variable assignment} $\alpha$:
	\begin{center}
...
\begin{block}{Properties of SLS Algorithms}
	Local search algorithms are \highl{incomplete}: \highlo{They cannot show unsatisfiability!}\\[1em]
	Challenges: 
	\begin{itemize}\setlength{\itemsep}{1ex}
...
\begin{block}{Consider a flip taking $\alpha$ to $\alpha'$}
	...
	\end{tabularx}
	\end{center}
	Typically, {breakcount}, {makecount} and {diffscore} are updated \highl{after each flip}
\end{block}
\pause
...
\begin{itemize}\setlength{\itemsep}{1em}
	\item Extremely \highl{successful and popular} in early days of SAT
	\begin{itemize}\setlength{\itemsep}{1ex}
		\item E.g., state of the art engine for \highl{automated planning} in the 90s
	\end{itemize}
	\item Today \highlo{outperformed} by sophisticated resolution-based solvers
	\begin{itemize}\setlength{\itemsep}{1ex}
		\item Faster, more reliable, and \highlo{complete!}
	\end{itemize}
	\item Still useful as a component in more complex solvers
	\begin{itemize}\setlength{\itemsep}{1ex}
		\item Part of \highl{algorithm portfolios}
...
Since $\phi$ satisfies both $C_1$ and $C_2$, it necessarily \highlo{satisfies at least one literal in $D$}. If $\phi$ satisfies $l$ then it satisfies some literal in $P_2$. Otherwise, if $\phi$ satisfies $\lnot l$ then it satisfies some literal in $P_1$.\\[1ex]
...
\begin{frame}{Resolution}
\begin{block}{Resolution is \highlow{Refutation} Complete}
\begin{itemize}\setlength{\itemsep}{1ex}
	\item If we manage to infer the \highlo{empty clause} from a CNF formula $F$, then $F$ is unsatisfiable.
	\item If $F$ is unsatisfiable, then there \highlo{exists a refutation by resolution}.
	\item \highlo{Not all possible} consequences of $F$ can be derived by resolution.
\end{itemize}
\end{block}
